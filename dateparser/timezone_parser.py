...
def is_timezone_token(token):
    ...
    space-padded token such as ``" est"`` while never matching a longer word
    that merely starts with a timezone abbreviation (e.g. ``"actualisé"`` is not
    treated as the ``ACT`` timezone).
    """
