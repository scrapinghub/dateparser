...
.. warning:: When this setting is enabled, discarded edge text is dropped even if it
   ...
   produce a date, e.g. ``'Chapter 12 March of the Penguins'``. A recognized timezone
   at the *end* of the string is kept and applied (e.g. ``'... 1:21 PM EST'``); a
   timezone at the *beginning*, or any timezone name the parser does not recognize, is
   discarded with the rest of the surrounding text. Enable this setting only when the
   input is expected to contain a date, and consider combining it with
